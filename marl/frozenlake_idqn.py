...
class MultiAgentFrozenLake:
    ...
    Каждый env = "агент".
    Это тот самый MultiEnvWrapper, о котором мы говорили.
    """
...
def train_idqn_frozenlake_multi(
    n_agents: int = 4,
    ...
    log_dir: str = "runs/frozenlake_idqn_multi",
):
    ...
    for episode in range(1, total_episodes + 1):
        ...
        for step in range(max_steps_per_episode):
            ...
            for i in range(n_agents):
                if random.random() < eps:
                    ...
                    a = int(torch.argmax(q_values[i]).item())
                actions.append(a)
            ...
            if len(replay) >= start_learning:
                ...
                with torch.no_grad():
                    ...
                    target = rewards_b + gamma * (1.0 - dones_b) * q_next_max
        ...
        for i in range(n_agents):
            writer.add_scalar(f"episode/agent_{i}_reward", episode_rewards_per_agent[i], episode)
        writer.add_scalar("episode/epsilon", eps, episode)

        if episode % 100 == 0:
            print(
                f"Episode {episode}/{total_episodes} | "
                f"mean_reward={mean_reward:.3f} | eps={eps:.3f}"
            )
